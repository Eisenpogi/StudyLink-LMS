...
$redirect = "../class_view.php?id={$class_assignment_id}&tab=assignments";
...
$due_timestamp = strtotime($due_date_input);

if ($due_timestamp === false) {
    header("Location: {$redirect}&error=invalid_due_date");
    exit();
}
...
if (!studylink_class_is_writable($conn, $class_assignment_id)) {
    redirect_with_flash(
        $redirect,
        'error',
        'This Academic Term is read-only. Assignments can only be created in the current term.'
    );
}
...
$escaped_due_date = mysqli_real_escape_string($conn, $due_date);
...
if ($attachment_source === 'drive') {
    ...
    $drive_item = mysqli_fetch_assoc($item_query);
    ...
    header(
        "Location: {$redirect}&" .
        ($insert ? 'success=created' : 'error=database')
    );
    exit();
}
...
$upload_directory =
    '../../assets/uploads/faculty_drive/' . $faculty_id . '/';
...
$escaped_original_filename =
    mysqli_real_escape_string($conn, $original_filename);

$escaped_file_path =
    mysqli_real_escape_string($conn, $public_file_path);
...
$assignment_insert = $drive_insert && mysqli_query($conn, "
    INSERT INTO assignments
    (
        class_assignment_id,
        faculty_drive_item_id,
...
if (!$drive_insert || !$assignment_insert) {
